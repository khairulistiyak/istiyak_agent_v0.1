...
export function useIdeMode({ workspacePath }: UseIdeModeOptions) {
  ...
  const handleOpenFile = useCallback(async (relPath: string) => {
    ...
    const binaryExtensions = [
      ...
      ".tar", ".gz", ".exe", ".dll", ".so", ".dylib", ".mp4", 
      ".mp3", ".wav", ".ico", ".woff", ".woff2", ".ttf", ".eot"
    ];
  ...
  const handleExecuteTerminalCommand = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (isDangerous) {
      const confirmed = window.confirm(`WARNING: The command "${cmd}" looks potentially dangerous. Do you still want to execute it?`);
      if (!confirmed) {
        setLogs(prev => [...prev, {
          time: new Date().toLocaleTimeString(),
