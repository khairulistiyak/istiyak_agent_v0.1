...
export const AuthModal = React.memo(
  ({ isOpen, onClose, token, userEmail, updateSettings }: AuthModalProps) => {
    ...
    // OAuth state
    const [oauthGoogleConfigured, setOauthGoogleConfigured] = useState<boolean | null>(null);
    const [oauthGitHubConfigured, setOauthGitHubConfigured] = useState<boolean | null>(null);
    ...
    useEffect(() => {
      if (!isOpen) return;
      fetch(`${SAAS_BASE}/api/auth/status`)
        .then((res) => res.json())
        .then((data) => {
          setOauthGoogleConfigured(data.google.configured);
          setOauthGitHubConfigured(data.github.configured);
        })
        .catch(() => {
          setOauthGoogleConfigured(false);
          setOauthGitHubConfigured(false);
        });
    }, [isOpen]);

    // Poll for token after OAuth window opens
    useEffect(() => {
      ...
      const timeout = setTimeout(() => setOauthPolling(false), 120000);
    ...
    const handleOAuthLogin = (provider: "google" | "github") => {
      const url = `${SAAS_BASE}/api/auth/${provider}`;
          ...
          {!token ? (
            ...
            <>
              <div className="flex flex-col items-center text-center space-y-1">
                ...
                <p className="text-[11px] text-cyber-textSecondary">
                  {authMode === "login"
                    ? "Enter details to access your companion"
                    : "Get started with 3 accounts per IP limitation"}
                </p>
              </div>

              <form onSubmit={handleAuthSubmit} className="flex flex-col space-y-3.5">
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold tracking-wider text-cyber-textSecondary">
                    Email Address
                  </label>
                  <Input
                    type="email"
              ...
              <div className="flex flex-col space-y-2 border-t border-cyber-cardBorder/30 pt-3">
                ...
                <button
                  ...
                  disabled={oauthGoogleConfigured === false}
                  className={`w-full py-1.5 rounded-lg text-[10.5px] font-semibold transition-all cursor-pointer flex justify-center items-center ${
                    oauthGoogleConfigured === false
                      ? "bg-[#1e1e1e] text-gray-500 cursor-not-allowed border border-[#2a2a2a]"
                      : "bg-[#ea4335]/15 hover:bg-[#ea4335]/25 border border-[#ea4335]/30 hover:border-[#ea4335]/50 text-white"
                  }`}
                  title={
                    oauthGoogleConfigured === false
                      ? "Google OAuth not configured. Add GOOGLE_CLIENT_ID to saas-backend/.env"
                      : "Sign in with Google"
                  }
                >
                  {oauthGoogleConfigured === null ? "Checking..." : "Google Account"}
                </button>

                <button
                  type="button"
                  onClick={() => handleOAuthLogin("github")}
                  disabled={oauthGitHubConfigured === false}
                  className={`w-full py-1.5 rounded-lg text-[10.5px] font-semibold transition-all cursor-pointer flex justify-center items-center ${
                    oauthGitHubConfigured === false
                      ? "bg-[#1e1e1e] text-gray-500 cursor-not-allowed border border-[#2a2a2a]"
                      : "bg-[#24292e]/40 hover:bg-[#24292e]/60 border border-[#24292e]/60 hover:border-white/30 text-white"
                  }`}
                  title={
                    oauthGitHubConfigured === false
                      ? "GitHub OAuth not configured. Add GITHUB_CLIENT_ID to saas-backend/.env"
                      : "Sign in with GitHub"
                  }
                >
                  {oauthGitHubConfigured === null ? "Checking..." : "GitHub Account"}
                </button>

                {oauthPolling && (
                  <div className="text-center text-[10px] text-cyber-primary animate-pulse pt-1">
            ...
            <div className="flex flex-col space-y-4 pt-2">
              <div className="flex flex-col items-center text-center space-y-1">
                ...
                <h2
                  className="text-sm font-bold text-white truncate max-w-[240px]"
                  title={userEmail}
                >
                  {userEmail}
                </h2>
              ...
              <div className="space-y-1.5 border-t border-cyber-cardBorder/30 pt-3 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium">
                    {isActiveLicense ? "SaaS Pro Developer" : "SaaS Free Tier"}
                  </span>
                </div>
